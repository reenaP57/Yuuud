...
+ (NSData *)keychainDataFromMatchingIdentifier:(NSString *)identifier
{
    NSData *valueData = [self searchKeychainCopyMatchingIdentifier:identifier];
    if (valueData) {
        return valueData;
...
+ (BOOL)createKeychainObject:(NSData *)value forIdentifier:(NSString *)identifier
{
    NSMutableDictionary *dictionary = [self setupSearchDirectoryForIdentifier:identifier];
    ...
    if (status == errSecSuccess) {
        ...
        return [self updateKeychainObject:value forIdentifier:identifier];
    } else {
        return NO;
    }
}

+ (BOOL)updateKeychainObject:(NSData *)value forIdentifier:(NSString *)identifier
{
