...
- (double)progress
{
	@synchronized(self)
	{
        if (sampleRate > 0 && (state == AS_STOPPING || ![self isFinishing]))
        {
            ...
            if (state != AS_PLAYING && state != AS_PAUSED && state != AS_BUFFERING && state != AS_STOPPING && state != AS_WAITING_FOR_QUEUE_TO_START)
            {
                NSLog(@"progress is: %f", lastProgress);
            ...
            if(isnan(lastProgress))
                lastProgress = CScreenWidth;
            
            NSLog(@"last progress: %f", lastProgress);
...
- (double)calculatedBitRate
{
	if (packetDuration && processedPacketsCount > BitRateEstimationMinPackets)
	{
        ...
        if([self.fileExtension isEqualToString:@"mp3"])
            return 10.0 * averagePacketByteSize / packetDuration;
        return 8.0 * averagePacketByteSize / packetDuration;
	}
...
- (void)stop
{
	@synchronized(self)
	{
		...
		{
            NSLog(@"Entered in if condition...");
			self.state = AS_STOPPING;
			stopReason = AS_STOPPING_USER_ACTION;
			err = AudioQueueStop(audioQueue, true);
			if (err)
			{
               // NSLog(@"Failed due to click faster");
                [self failWithErrorCode:AS_AUDIO_QUEUE_STOP_FAILED];
				return;
			}
		}
		else if (state != AS_INITIALIZED)
		{
            ...
            //NSLog(@"Entered in else if condition...");
...
- (void)handleInterruptionChangeToState:(NSNotification *)notification
{
	...
	{
        NSLog(@"Interruption if condition... ");
	...
	{
        NSLog(@"Interruption else condition... ");
